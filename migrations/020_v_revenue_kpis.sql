-- =============================================================================
-- Migration 020 — v_revenue_kpis
-- =============================================================================
-- M2.3.2 Revenue Phase 1 — KPI strip data source.
--
-- Returns 6 metrics over the 12-month rolling window:
--
--   ANCHOR (top row):
--     1. top_3_customer_share_pct  — concentration risk indicator
--     2. fx_invoiced_share_pct     — FX exposure on the receivable side
--     3. active_customer_count     — distinct customers active in last 12m
--
--   CONTEXT (bottom row):
--     4. core_revenue_share_pct    — core / total (excludes yarn_resale)
--     5. contra_share_pct          — total_contra / gross_revenue
--     6. top_3_customer_share_delta_pp + reference months
--          → percentage-point shift in top-3 customer share
--            (latest complete month vs prior complete month).
--          Positive = concentration rising, negative = dispersing.
--
-- Scope: core_product_sales + outsourced_service_revenue (yarn_resale excluded)
--        — same scope as v_top_customers_overall and v_monthly_revenue_core.
-- =============================================================================
...
window_bounds AS (
    SELECT
        ...
        -- Latest complete month: drop the in-progress trailing month
        CASE
...
-- Total CORE revenue in 12m window (yarn_resale excluded)
core_total_12m AS (
...
-- FX-invoiced rows (USD or EUR) — TL value
fx_share AS (
    SELECT
...
-- Core vs total (gross) revenue. Total here = ALL sales rows in window
-- including fason and yarn_resale. Core / total tells how much revenue is
-- the "real" product business.
...
-- Contra revenue (returns + discounts) — these are negative-amount rows
-- in the sales fact table. We sum the absolute magnitude.
contra_12m AS (
    SELECT
        SUM(CASE
              WHEN s.business_bucket IN ('returns_revenue', 'discounts_revenue')
              THEN ABS(s.net_tutar_y)
              ELSE 0
            END)::numeric AS contra_amount_tl,
        SUM(CASE
              WHEN s.net_tutar_y > 0 THEN s.net_tutar_y ELSE 0
            END)::numeric AS gross_positive_tl
    FROM fact_sales_lines_clean s
    CROSS JOIN window_bounds wb
    WHERE s.fatura_tarihi >= wb.min_date
),

-- ── Concentration shift: top 3 share in latest complete month vs prior ──
...
SELECT
    ...
    ROUND(
        100.0 * (SELECT contra_amount_tl FROM contra_12m)
              / NULLIF((SELECT gross_positive_tl FROM contra_12m), 0),
        2
    ) AS contra_share_pct,

    -- KPI 6 — Top 3 customer share Δ (percentage points, latest vs prior)
    ROUND(
...
'M2.3.2 — Revenue Phase 1 KPI strip data. Single-row view, returns 6 metrics
over the 12-month rolling window plus a concentration shift (KPI 6 = top-3
customer share Δ in percentage points, latest complete month vs prior).
Scope: core_product_sales + outsourced_service_revenue (yarn_resale excluded).';
